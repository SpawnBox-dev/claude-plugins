...
## 2026-09-06 - The channel can say where a message died: a per-emit log, identity-based routing, and a loss report that admits what it cannot see (0.69.8 + 0.69.9, WI 6cf7437a)

**Change.** (1) `agent_channel_emitlog.ts`: one record per routing decision - `emit` / `sent` / `send_failed` / `unknown_sender` / `paragraph_filtered` - carrying receiver, sender, the SOURCE BYTE OFFSET of the originating transcript line, and an `emit_id` that also rides in the notification meta, so it renders as an attribute on the `<channel ...>` tag in the RECEIVER's transcript. (2) Sender resolution now consults IDENTITY, not liveness. (3) A loss report in `system_status`, built on both detectors, silent when clean.

**Why an id and not a timestamp or a substring.** Two candidate explanations for every loss produced identical evidence on every surface that existed: the producer consumed the line and never called `notification()`, or it called it and Claude Code never enqueued it. The offsets table cannot separate them - it advances by bytes CONSUMED and `processEvent` has paths that consume without emitting. The receiver's `queue-operation` rows cannot either - `enqueue` is logged only once the handler runs, so both cases render as no row at all. The `emit_id` joins the two halves exactly; the warden's content-substring probe demonstrably returned different answers at different match lengths (reproduced: a row matching at 30 chars and failing at 60 and 120, entity escaping in the rendered tag being the likely cause).

**Why identity, not liveness (the actual fix).** `removeSession` is an unconditional `DELETE FROM sessions` issued by a PEER at `agent_channel.ts:1580` against the table the whole fleet shares. One session's departure hysteresis therefore deletes a peer's row for EVERYONE, and until that peer's next 30s heartbeat re-inserts it, every watcher fails to resolve it, consumes its transcript lines, advances past them and emits nothing. Measured signature: contiguous RUNS of loss with clean deliveries between them, because the blindness is a time window, not a per-message dice roll. A plugin reload is the most reliable trigger, not a separate mechanism - confirmed to the millisecond on a pair of lines written 233ms and 485ms before that MCP died. Resolution now unions `currentRoster`, `knownSessions` (process-local, so a peer's reap cannot empty it) and an unfiltered registry read; the heartbeat filter keeps deciding who is ADDRESSABLE and who is LIVE, never who is allowed to have SPOKEN.

**Rejected.** (a) *Resolving against "the durable registry row"* as first ruled - that row does not exist; the reap deletes precisely the record such a fix would read. (b) *Holding the offset until a sender resolves* - the real fix in principle, but a transcript from a plain `claude` run that never registered would stall forever. The union preserves the no-wedge property: a session in none of the three sources is still skipped and its offset still advances. (c) *Bounding the transcript read and the emit log by two byte constants* - two windows over files with different growth rates cannot agree and drift apart unwatched, and a transcript window narrower than the log window MANUFACTURES FALSE GAPS. Overcounting is the costlier error here: undercounting loses a signal, overcounting destroys the credibility of the only instrument that separates real loss from noise. The tail instead reports the earliest instant it genuinely covers and the sent-set filters to match, so both halves agree by construction.

**KNOWN LIMIT, AND IT MUST NOT BE READ AS AN ALL-CLEAR.** The report detects discards and enqueue failures **for messages the producer actually sent**. It CANNOT detect a message the producer never emitted: the denominator is built from `event === "sent"` records, so a line that produced no `emit` and no `sent` never enters the set and can never appear as a gap. This is not hypothetical - a ~69KB, ~15-minute span of one session's transcript was consumed by all four watchers on 2026-09-06 02:11-02:26Z with no emit, no sent and no enqueue row anywhere, and the report reads CLEAN for exactly that span. **A silent loss report is therefore not proof of a healthy channel.** Two unlogged paths in `processFile`/`processEvent` can consume a line and record nothing: a `JSON.parse` failure (`catch { continue }`, offset already advanced) and `filterEvent` returning null. Instrumenting those at low volume - every parse failure, and `filterEvent`-null only for assistant entries carrying non-empty text, a count that should be zero - is the next step and is NOT in this release.

**`findCounterGaps` and `parseEmitId` are deliberately NOT on a runtime path** and read 0 in the bundle. `summarizeLoss` has the log and answers exactly by set difference; sequence inference reported an id that was emitted-but-never-sent as a harness drop, counting one failure as two. They remain as the tested reference for a consumer holding only a receiver's transcript, and they encode the restart-epoch trap: the counter resets on MCP restart, so raw counters must never be compared across epochs.

**Two defects caught by this work's own tests and controls, both pinned.** (i) `extractSeenEmitIds` matched only the unescaped attribute; on disk the tag sits inside a JSON string, so the bytes are `emit_id=\"...\"` and the pattern returned `[]` - reading as ABSENCE rather than as a broken matcher. (ii) An unreadable OR unanchorable seen-window left the seen-set empty, making every sent id look missing and reporting a TOTAL fabricated loss with `measured` true - worse than a crash, because the block still produced output and a broken instrument impersonated a finding. Both now route to an explicit UNMEASURED that refuses silence and refuses a number. `earliestTimestamp` requires Z-suffixed UTC, because the window filter compares ISO strings and a `+02:00` form would sort by its literal digits and silently shift the count.

**Test additions.** `tests/engine/emit-log.test.ts` (11), `tests/engine/channel-loss-detectors.test.ts` and `tests/integration/identity-routing.test.ts` (43 together). Load-bearing arms: an emit-without-sent is distinguishable from an emit-with-sent; a restart must not manufacture losses; a message written while the sender's row is deleted still routes; identity resolves a reaped session while the liveness roster does not (the RED arm - it asserts the OLD expression still fails); an unregistered transcript is skipped, its offset advances, and nothing wedges across ticks. The window arms drive the REAL file path, because the bug lived in how the window was CONSTRUCTED and a test building its own window would have passed throughout.

**Neither the item nor the mechanism closes on this publish.** Fix (b) covers 2 of 5 measured losses; three remain unexplained. Whether a second mechanism operates past the plugin boundary is NEITHER ESTABLISHED NOR REFUTED - the evidence first offered for it was on the wrong instrument (`mcp-lifecycle.log` writes "alive" every 300s, while roster membership is decided by `last_heartbeat_at`, a 30s timer read by a 90s filter, so process continuity says nothing about heartbeat freshness). The forward discriminator needs no lifecycle log: a loss WITH an `unknown_sender` record is the identity mechanism; a loss with `emit` + `sent` and no enqueue row at the receiver is past our boundary.

**POST-PUBLISH CORRECTION, measured at the first reload with 0.69.9 live (02:50Z).** The identity fix does NOT cover a fleet-wide reload, and the claim that it "covers the restart window" was wrong for the case everyone actually hits. `stop()` calls `removeOwnSession`, so **every session DELETES ITS OWN registry row on shutdown** - during a reload the row is genuinely absent, and a freshly-started receiver's `knownSessions` is empty too, so all three identity sources are empty at once. Verified rather than inferred: the `unknown_sender` `detail` string differs per build, and 6 discard records at that reload carry the 0.69.9 wording. Coverage precisely: a peer reap of a *live* session is COVERED; a sender restarting while the receiver stays up is COVERED (the receiver's `knownSessions` still resolves it); a **simultaneous fleet reload is NOT**. The channel did settle immediately afterward - 11 consecutive `emit`/`sent` records with zero discards - so the loss is confined to the restart window itself. Closing it needs identity to survive a process exit: a tombstone or `exited_at` column instead of the shutdown DELETE, leaving the row to carry identity while `last_heartbeat_at` remains the sole liveness signal. Not built. Note that `removeOwnSession` already carries an instance guard so a reload's new process does not delete the row it just took over, which suggests the shutdown DELETE was always about avoiding stale rows rather than about identity.

**Shipped:** `74a1cbb` (0.69.8), `c804c73`, `a67944a`, `7156cb5` (0.69.9). PA diff review approved 01:58Z, blocker-corrected and re-cleared 02:29Z. Live-verified 02:50Z.
...
- **RAID framing** (both payloads): one tight sentence framing warm, recently-uncompacted peers as striped redundancy for context - any agent (especially PA) rebuilds coherence by aggregating from them rather than trusting its own lossy summary. (Full principle goes into the operating-contract layer separately.)
- **PreCompact prompt DELETED (not softened).** `handlePreCompact` now returns NO `systemMessage` at all; it only writes the `compacting_<sid>` Stop-suppression marker (unchanged) and banks a synthetic mini-checkpoint (`current_task` + recent notes + in-flight work_items, ISO-ts) into `plugin_state` (`precompact_cp_<sid>`). That synthetic snapshot is now the **first-class** pre-compact capture mechanism: `handleSessionStartCompact` prefers a FRESH synthetic outright (30-min bound guards a lingering prior-compaction snapshot), falling back (no fresh synthetic) to THIS session's own latest real checkpoint (`notes.source_session == sid`), and only to the global-latest checkpoint as a last resort when this session has none - so this session's own boundary state is never shadowed by another session's newer checkpoint, in the synthetic-present case OR the degraded fallback.
- **Regular-checkpoint cadence nudge (new, replaces the dead prompt as the compliance path).** Since point-of-compaction capture is impossible, `handleUserPromptSubmit` now nudges agents to checkpoint DURING normal work. It is explicitly NOT an every-turn nag: it fires only when turns-since-`save_progress` AND substantive-uncaptured-activity BOTH cross a role-aware bar, and each fire raises its own bar (level) so successive nudges are spaced further apart and worded more urgently (gentle -> firm -> URGENT). `save_progress` resets all cadence state; PA thresholds are tighter than SA (PA context loss is the worst case). Tracking is in `handlePostToolUse` (save resets; edits/knowledge-captures bump; reads/lookups don't).

**Why.** Jarid-ratified (overrides 2026-07-11). Pre-existing gap: when PA compacted, nothing fleet-ward fired and the just-compacted agent got SA-tuned guidance (it was even told a peer-backstop was emitted "on its behalf" when `buildPeerBackstopEvent` had returned null for the PA-self case). The server already knows each session's role - use it. PreCompact's long-standing "call save_progress NOW" `systemMessage` was **non-actionable AND actively harmful**: Claude Code's PreCompact hook is a synchronous decision point with **no model turn between it and compaction** (verified 2026-07-11 vs code.claude.com/docs/en/hooks - "no model turn between the hook firing and compaction"). Non-actionable because the agent can never act on it pre-compaction (same trigger-design defect class as 5d1c20fc); actively harmful because the message can therefore only reach the model on the NEXT turn - AFTER compaction - where "save_progress now" induces the agent to checkpoint the already-degraded, incomplete post-compaction context. Jarid: future agents must not inherit code that looks functional but structurally cannot fire. So the prompt is deleted, capture goes fully deterministic (synthetic snapshot), and regular-checkpointing (the only capture that CAN work) is encouraged via the cadence nudge.

**Rejected.** (a) Reusing `post_compact_recovery` with a role flag for the PA direction - the existing consumer hard-assumes compacted=SA, recipient=PA; a distinct `pa_compact_recovery` keeps the proven SA->PA path byte-identical. (b) Keeping/softening PreCompact's `systemMessage` - non-actionable and post-compaction-harmful (see Why); deleted outright. (c) Fresher-of-{real,synthetic} comparison (the pre-override design) - a global-latest checkpoint can belong to ANOTHER session and would shadow this session's own boundary state; the synthetic is this session's, so prefer it when fresh. (d) An every-turn "please checkpoint" nudge - trains agents to ignore it (the exact 5d1c20fc defect); the both-gates + per-level-widening escalation is the anti-noise design. (e) Making `role`/`peers` required composer params - would break every existing caller/test; they default to pre-2ad3240e subordinate behavior.
...
**Cadence thresholds (initial, tunable).** PA `{turnBase 6, turnStep 5, actBase 5, actStep 5}`; SA `{turnBase 10, turnStep 6, actBase 12, actStep 8}`. The invariants (both-gates, per-level widening, reset-on-save, PA-tighter) are load-bearing; the exact constants are not.

**Test additions.** ~40 new/updated tests in `tests/tools/hook_event.test.ts` (prime fleet-check-in + roster, SA check-in + lateral roster, high-loss-zone both directions, RAID framing both payloads, roster bounding, `buildPaCompactAdvisoryEvent` payload, `composePrecompactSnapshot` bounding/safety, PreCompact returns no systemMessage + banks snapshot, synthetic-first-class + stale-synthetic guard + fallback, PreCompact->SessionStart end-to-end, and the cadence nudge: silent-until-both-gates, reads-don't-count, save-resets, spaced-and-escalates-to-URGENT). Full suite: **618 pass, 0 fail**; `tsc --noEmit` clean. Existing `hook_envelope.test.ts` `ALLOWED_HSO_EVENT_NAMES` semantics unchanged (SessionStart still non-HSO, delivers via top-level `systemMessage`; PreCompact now emits the empty fast-path envelope).

**P2 review amendment (SA-0c230dcf, 2026-07-11).** Reviewer verified all four ratified items against the diff (PreCompact returns `{}` + marker + synthetic bank; synthetic first-class with 30-min bound + session-scoped read; cadence nudge provably cannot fire every turn - each fire raises both bars by a full step; RAID line in both role payloads) and the symmetric `pa_compact_recovery` emit/render path. One gap found + fixed inline: the no-fresh-synthetic fallback was global-latest, so another session's newer checkpoint could shadow this session's own in the degraded path (contradicting the "never shadowed" claim). Fix: prefer `source_session == sid` first, global-latest only as last resort (+ regression test). Everything else verified correct.
...
**Why.** CC records launch-time names (`--name`, used by every scripted/PA launch) nowhere durable: the live `~/.claude/sessions/<pid>.json` dies with the process, the agent-channel registry purges on departure, and the transcript only ever contains `Session renamed to:` lines from in-app `/rename`. So `-Resume <name>` worked for renamed sessions and hard-failed for launcher-named ones (user-reported 2026-06-11; also the reason c912760c's rule said resume by raw uuid). The hook's SessionStart payload carries `session_title` (verified live), making the hook the natural producer of a durable map.

**Rejected.** Seeding a `/rename` command as the launch prompt - consumes the seed slot (whose purpose is starting real work), depends on unverified positional-arg slash dispatch, and only helps launches that remember the ritual. Storing the map in agent_channel.db - adds a sqlite3 dependency + lock contention to a bash hook that today only writes flat files; the append-only tsv has no read-modify-write race by construction. Merging Layer-2 results into Layer-1 disambiguation - changes existing resolution behavior for renamed sessions; out of scope (Layer 2 only fires where Layer 1 errors today, zero regression surface).

**Test additions.** Fixture-tested end-to-end (no suite coverage exists for bash hooks / ps1 launchers): hook writes startup+resume rows, skips unsafe-char and missing titles, keeps valid JSON output + legacy active-session writes; resolver resolves single-id names through the real script (launch-stubbed), lists candidates + exit 1 on multi-id, combined error on unknown; malformed map lines skipped. All four ps1 files parser-validated.

**Shipped:** v0.30.56. Spawnbox project-root launchers mirrored same-commit. PLUGIN_VERSION is runtime-read from package.json, so no dist rebuild. Note: the map only populates as sessions boot on >=0.30.56 - pre-existing dead sessions remain uuid-resume-only.
