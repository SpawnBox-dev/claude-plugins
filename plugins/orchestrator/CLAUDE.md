...
3. **State the alert's own track record inside the alert.** `ingress_suspect` now says *"Every firing of this alert so far has been a false alarm"* - and PA's observation from the fifth firing is that this is **why** nobody treated it as urgent: **an alert that states its own false-positive rate gets read as a question rather than as news.** A reader can only calibrate on a base rate they can see, and no one goes looking for one. Transferable to any advisory whose precision is known and imperfect.
