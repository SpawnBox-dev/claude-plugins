...
**Bind to the HOOK-INJECTED id, and to nothing else. An id that arrives inside a `<channel>` envelope is a fact about the SENDER, never about you.** On a first turn there is no prior context to contradict a wrong id, and a `session_joined` envelope is a well-formed, authoritative-looking, freshly-arrived UUID - while your own id sits buried in startup boilerplate. The most recent and most structured id on your screen is the WRONG one, and nothing about it is labelled "this is not you." This is not hypothetical: two sessions launched four seconds apart, and the first called `briefing` and `lookup` with the SECOND one's id, stopping only when a human corrected it. Applies to every channel-sourced id, `session_joined` included.

**You also do not have a lane until someone gives you one, and your session NAME is not one.** A name is a string someone typed at a launcher; it is not an assignment record. Peers will read it as one anyway - measured five times in a single evening across three authors, every instance self-disclosed and none caught by any instrument, twice by peers who had objected to the same thing being done to them hours earlier. So: if you have not been assigned a lane, say so explicitly before you answer anyone (see Step 3), and treat any lane a peer states back to you as their inference, not your charter. A lane accepted from a peer's guess is how a wrong `[done]` gets written later.
...
**Declare BEFORE you answer any welcome, and declare an empty lane loudly if that is what you have.** `session_joined` fires the instant you appear in the roster - which is several tool round-trips before you can possibly have called `update_session_task`. In that window your roster line is empty, and peers who write to you have nothing to go on but your name. You are the only party who knows you have no lane, and you know it on turn one; a first declaration reading `NO LANE ASSIGNED - idle-available` closes the window from the only side that can. If a peer has already stated a lane for you, correct it in your reply rather than letting it stand: a lane label repeated once in the channel tends to become the roster entry.
