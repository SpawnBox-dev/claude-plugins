...
**Cross-session events are auto-delivered.** Agent-channel notifications (`<channel ...>` injections from peer sessions) arrive in your context in real-time at every model turn - no inbox to drain at wrap-up. If you addressed peers via `@PA` / `@SA-<id8>` / `@all` in your terminal output this session, the filewatcher already routed those events to them. No follow-up needed from you.
