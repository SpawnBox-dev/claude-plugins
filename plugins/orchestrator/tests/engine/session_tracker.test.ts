...
describe("SessionTracker", () => {
  ...
  test("annotateResult marks already-sent notes", () => {
    tracker.registerSession("sess-1");
    tracker.logSurfaced("sess-1", "note-1", 1, "fresh");
    // Need a note in the notes table for access_count
    const ts = new Date().toISOString();
    db.run(`INSERT INTO notes (id, type, content, keywords, confidence, last_validated, resolved, created_at, updated_at, access_count) VALUES (?, ?, ?, '', 'medium', ?, 0, ?, ?, 0)`,
